...
export async function getLowStockProducts(limit = 5) {
  const [rows] = await pool.query(
    `SELECT id, name, sku, quantity, minimum_stock AS minimumStock
       FROM products
      WHERE active = TRUE AND quantity <= minimum_stock
...
export async function getRecentMovements(limit = 8) {
  const [rows] = await pool.query(
    `SELECT m.id, m.type, m.quantity, m.created_at AS createdAt,
