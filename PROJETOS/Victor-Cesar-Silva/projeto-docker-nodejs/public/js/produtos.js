...
// Uma funcao para os dois modos: openModal(null) cria, openModal(produto) edita
...
// Tres formas de fechar: botao, fundo escuro e Escape
document.querySelectorAll("[data-close]").forEach((button) => {
  button.addEventListener("click", closeModal);
});

modal.addEventListener("click", (event) => {
  // Sem esta checagem, clicar DENTRO da caixa branca tambem fecharia
...
// Early return: a ordem define a prioridade do status
...
function currentFilters() {
  ...
  return {
    ...
    // Checkbox marcado vira "on"; desmarcado nem aparece no FormData
...
async function loadCategories() {
  ...
  // A mesma lista nos dois selects, com primeira opcao diferente
...
filtersForm.addEventListener("reset", () => {
  // O evento reset dispara ANTES de o navegador limpar os campos
  setTimeout(loadProducts, 0);
});

// Delegacao de eventos: ouvimos no container, que existe desde o inicio.
// Assim os botoes criados depois (ao filtrar) tambem funcionam.
...
form.addEventListener("submit", async (event) => {
  // Sem isto o navegador recarregaria a pagina inteira
  event.preventDefault();
  ...
  const payload = {
    name: data.get("name"),
    sku: data.get("sku"),
    // "Sem categoria" chega como "" e o backend espera null
    categoryId: data.get("categoryId") || null,
    // Todo campo de formulario vira string, mesmo com type="number"
