#!/bin/bash
# Script de evidencia da atividade 16 - Parando serviços com Docker Compose
...
{
  echo "# Evidencia de execucao - Atividade 16: Parando serviços com Docker Compose"
  echo
  ...
  echo
  echo '```'
  printf '%s\n' '$ docker compose -f ../at15-compose/docker-compose.yml down'
  docker compose -f ../at15-compose/docker-compose.yml down
  echo
  printf '%s\n' '$ docker compose -f ../at15-compose/docker-compose.yml ps -a'
  docker compose -f ../at15-compose/docker-compose.yml ps -a
  echo
  echo '```'
  echo
